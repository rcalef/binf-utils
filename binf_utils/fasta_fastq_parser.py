...
class FastqSeq(FastaSeq):
    ...
    def write_fastq(self, output, phred = 33):
        print("@%s %s\n%s\n+\n%s" % (self.identifier,
                                     self.comment,
                                     self.sequence,
                                     str(map(lambda x:chr(x+phred),self.scores))),
                                    file = output)
...
class FastaParser(BaseFastxParser):
    ...
    def __init__(self, filename, replace_invalid = False):
        self._fasta_fh = open(filename)
        self.replace_invald = replace_invalid

    def __next__(self):
        return self._get_next_fasta_seq()

    def _get_next_fasta_seq(self):
        seq_id = ""
        comment = ""
        line  = ""
        lines = []
        ...
        for line in self._fasta_fh:
            if line.startswith('>'):
                return FastaSeq(identifier,comment,"".join(lines))
...
class FastaWithQualityParser(FastaParser):
    ...
    def __next__(self):
        found_fasta = True
        try:
            fasta = self._get_next_fasta_seq()
        except StopIteration:
            found_fasta = False

        found_qual = True
        try:
            qual = self._get_next_quals()
        except StopIteration:
            found_qual = True
    ...
    def _get_next_quals(self):
        ...
        for line in self._fasta_fh:
            if line.startswith('>'):
                return FastqSeq(identifier,comment,"",scores)

            # Read the scores, raise exception if invalid score 
            parts = line.split()
            for part in parts:
                if not part.is_digit() or int(part) < 0:
        ...
        return FastqSeq(identifer,comment,"",scores)
    ...
    def __next__(self):
        ...
        try:
            ...
            for pos,symbol in enumerate(sequence):
                if symbol not in fastq_allowed_bases:
                    print("WARNING Encountered symbol not "
                          "in alphabet: %s\nSequence: %s, position: %d"
                    % (char,identifier,pos),file=sys.stderr)
            ...
            score_line = next(self._fastq_fh)
        except StopIteration:
        ...
        for char in score_line:
            ...
            scores.append(score)
        return FastqSeq(seq_id, comment, sequence, scores)
